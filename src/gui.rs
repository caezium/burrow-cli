...
pub fn locate() -> Option<PathBuf> {
    // Test/dev seam: BURROW_GUI_APP pins the app location outright — a missing path means
    // "not installed" (lets tests exercise the miss without ever launching a real app).
    if let Ok(p) = std::env::var("BURROW_GUI_APP") {
...
pub fn launch(path: &Path) -> Result<(), Failure> {
    let st = Command::new("open")
        .arg(path)
        .status()
        .map_err(|e| Failure::io("failed to run `open`", &e))?;
    if st.success() {
        Ok(())
    } else {
        Err(Failure::process_failed(format!("`open` exited {st}")))
    }
}

/// Install the Homebrew cask (interactive — brew output streams to the terminal).
pub fn install() -> Result<(), Failure> {
    eprintln!("burrow gui: installing via Homebrew — brew install --cask {CASK}");
    let st = Command::new("brew")
        .args(["install", "--cask", CASK])
        .status()
        .map_err(|e| Failure::io("failed to run `brew` (is Homebrew installed?)", &e))?;
    if st.success() {
        Ok(())
    } else {
        Err(Failure::process_failed(format!(
            "`brew install --cask {CASK}` exited {st}"
        )))
    }
}
