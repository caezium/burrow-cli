...
pub fn trash_each(paths: &[String], deleter: &dyn Delete, mut on_item: impl FnMut(Value)) -> usize {
    let mut failed = 0;
    for p in paths {
        let r = deleter.delete(p);
        failed += usize::from(r.is_err());
...
pub fn trash_all(paths: &[String]) -> Result<Value, Failure> {
    trash_all_with(paths, &SystemTrash)
}

fn trash_all_with(paths: &[String], deleter: &dyn Delete) -> Result<Value, Failure> {
    let mut results = Vec::new();
    let failed = trash_each(paths, deleter, |e| results.push(e));
    let result = json!({ "applied": true, "destination": destination(), "trashed": results });
    if failed > 0 {
        Err(
            Failure::error(format!("failed to trash {failed} of {} paths", paths.len()))
                .with_details(result),
        )
    } else {
        Ok(result)
...
pub fn plan_paths(json: &str) -> Result<Vec<String>, String> {
    let mut v: Value = serde_json::from_str(json).map_err(|e| format!("bad plan JSON: {e}"))?;
    if v.get("ok").is_some() {
        if v.get("ok").and_then(Value::as_bool) != Some(true)
            || v.get("command").and_then(Value::as_str) != Some("trash")
        {
            return Err("plan must be a successful trash preview".into());
        }
        v = v
            .get("data")
            .cloned()
            .ok_or("plan envelope is missing data")?;
    }
    let read_path = |value: Option<&Value>| {
        value
            .and_then(Value::as_str)
            .filter(|path| !path.trim().is_empty() && !path.contains('\0'))
            .map(String::from)
            .ok_or_else(|| "every plan entry must contain a non-empty path string".to_string())
    };
    if let Some(items) = v.get("would_trash").and_then(Value::as_array) {
        items.iter().map(|it| read_path(it.get("path"))).collect()
    } else if let Some(arr) = v.as_array() {
        arr.iter().map(|path| read_path(Some(path))).collect()
    } else {
...
mod tests {
    ...
    fn trash_moves_file_to_trash() {
        ...
        let v = trash_all(&[f.to_string_lossy().into_owned()]).unwrap();
    ...
    fn trash_each_streams_one_result_per_path_in_order() {
        ...
        let error = trash_all_with(&a(&["/x", "/bad", "/y"]), &Fake).unwrap_err();
        let envelope: Value =
            serde_json::from_str(&crate::output::error_envelope("trash", "native", &error))
                .unwrap();
        assert_eq!(
            envelope["error"]["details"]["trashed"]
                .as_array()
                .unwrap()
                .len(),
            3
        );
        assert_eq!(envelope["error"]["details"]["trashed"][1]["ok"], false);
    }
}
