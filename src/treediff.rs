...
pub fn scan_tree(root: &Path, max_depth: usize) -> Result<Vec<DirSize>, Failure> {
    let mut out = Vec::new();
...
fn accumulate(
    ...
    let entries = std::fs::read_dir(dir)
        .map_err(|e| Failure::io(format!("cannot scan {}", dir.display()), &e))?;
    for e in entries {
        let e =
            e.map_err(|e| Failure::io(format!("cannot read an entry of {}", dir.display()), &e))?;
        let meta = std::fs::symlink_metadata(e.path()).map_err(|error| {
            Failure::io(format!("cannot inspect {}", e.path().display()), &error)
        })?;
...
pub fn save_scan(root: &str, dirs: &[DirSize]) -> Result<i64, Failure> {
    let ts = crate::metrics::now_secs();
    let dir = scan_dir();
    std::fs::create_dir_all(&dir)
        .map_err(|e| Failure::io(format!("cannot create {}", dir.display()), &e))?;
    ...
    std::fs::write(
        &file,
        serde_json::to_string(&sf).map_err(|e| Failure::error(e.to_string()))?,
    )
    .map_err(|e| Failure::io(format!("cannot write {}", file.display()), &e))?;
    Ok(ts)
}
...
mod tests {
    ...
    fn scan_tree_counts_files() {
        ...
        let scan = scan_tree(&dir, 2).unwrap();
        let root = scan
            .iter()
