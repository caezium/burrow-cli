...
- a locally-generated install id (**not** derived from hardware): 16 bytes from
  `/dev/urandom` on macOS/Linux; Windows has no `/dev/urandom`, so there it is the portable
  fallback — 128 bits from two OS-seeded `RandomState` hashers (random, no timestamp)
...
```sh
burrow telemetry off       # persistent (~/.burrow/telemetry.json; %APPDATA%\Burrow\telemetry.json on Windows)
